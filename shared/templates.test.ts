...
describe("Templates", () => {
  ...
  describe("extractPlaceholders", () => {
    ...
    it("reports a repeated token once", () => {
      expect(extractPlaceholders("[Date] ... [Date] ... [Date]")).toEqual([
        "Date",
      ]);
    });
    ...
    it("ignores markdown checkboxes", () => {
      expect(
        extractPlaceholders("- [ ] Task one\n- [x] Task two\n- [X] Task three")
      ).toEqual([]);
    });

    it("ignores markdown link labels", () => {
      expect(
        extractPlaceholders("See [the docs](https://example.com) for [Details]")
      ).toEqual(["Details"]);
    });

    it("trims surrounding whitespace in a token", () => {
      expect(extractPlaceholders("[  Project Name  ]")).toEqual([
        "Project Name",
      ]);
    });
  ...
  describe("placeholder tokens are unambiguous", () => {
    ...
    it("deliberately reuses a token where it means the same thing", () => {
      ...
      // which is the point.
      const plan = applyTemplateValues(
        getTemplateById("project-plan")!.content,
        {
      ...
      const meeting = applyTemplateValues(
        getTemplateById("meeting-notes")!.content,
        {
  ...
  describe("suggestPlaceholderValue", () => {
    ...
    it("suggests a date, time and year", () => {
      expect(suggestPlaceholderValue("Date", now)).toBe(
    ...
    it("is case insensitive", () => {
      expect(suggestPlaceholderValue("date", now)).toBe(
        suggestPlaceholderValue("DATE", now)
      );
    });

    it("suggests today for role-named start dates", () => {
      expect(suggestPlaceholderValue("Start Date", now)).toBe(
        now.toLocaleDateString()
      );
      expect(suggestPlaceholderValue("Meeting Date", now)).toBe(
        now.toLocaleDateString()
      );
      expect(suggestPlaceholderValue("Today's Date", now)).toBe(
        now.toLocaleDateString()
      );
    });
  ...
  describe("applyTemplateValues", () => {
    it("substitutes every occurrence of a filled token", () => {
      const out = applyTemplateValues("[Date] and again [Date]", {
        Date: "4 March",
      });
    ...
    it("leaves a token alone when no value was given", () => {
      expect(applyTemplateValues("[Name] met [Client]", { Name: "Rey" })).toBe(
        "Rey met [Client]"
      );
    });
