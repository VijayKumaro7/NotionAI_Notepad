...
import { SHORTCUTS, groupShortcutsByCategory, formatKeys, ShortcutCategory } from '@/lib/shortcuts';
...
export default function ShortcutsModal({ isOpen, onClose }: ShortcutsModalProps) {
  ...
  const categories: (ShortcutCategory | 'All')[] = ['All', 'Navigation', 'Editing', 'Formatting', 'General', 'Search'];
      ...
      <div className="w-full max-w-3xl max-h-[80vh] bg-white dark:bg-slate-900 rounded-xl shadow-2xl flex flex-col animate-scale-in">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700">
          <div>
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">Master these shortcuts to work faster</p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="space-y-4">
              {selectedCategory === 'All'
                ? categories
                    .filter((cat) => cat !== 'All')
                    .map((category) => {
                      const categoryShortcuts = grouped[category as ShortcutCategory].filter((s) =>
                        filteredShortcuts.includes(s)
                      );
        ...
        <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 rounded-b-xl">
          <p className="text-xs text-slate-600 dark:text-slate-400">
            💡 Tip: Press <kbd className="px-2 py-1 bg-slate-200 dark:bg-slate-700 rounded font-mono text-xs">Cmd+?</kbd> anytime to open this help
          </p>
        </div>
      </div>
    </div>
