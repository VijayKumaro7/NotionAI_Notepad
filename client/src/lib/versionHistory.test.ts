...
describe('Version History Feature', () => {
  ...
  describe('createNoteVersion', () => {
    ...
    it('should record change type correctly', async () => {
      ...
      const editVersion = await createNoteVersion(testNote.id, testNote, 'edit');
      expect(editVersion.changeType).toBe('edit');

      const autoSaveVersion = await createNoteVersion(testNote.id, testNote, 'auto-save');
      expect(autoSaveVersion.changeType).toBe('auto-save');

      const restoreVersion = await createNoteVersion(testNote.id, testNote, 'restore');
  ...
  describe('restoreNoteVersion', () => {
    it('should restore a note to a previous version', async () => {
      ...
      // Update note
      const updatedNote = { ...testNote, title: 'Updated Title', content: 'Updated content' };
