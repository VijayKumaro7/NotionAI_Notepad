...
describe("hashPassword", () => {
  it(
    "accepts the password it just hashed",
    async () => {
  ...
  it(
    "gives two different hashes for the same password",
    async () => {
  ...
  it(
    "records the parameters it used",
    async () => {
...
describe("verifyPassword on input it should not trust", () => {
  it.each([
    ...
    async (_label, stored) => {
      await expect(verifyPassword("any password at all", stored)).resolves.toBe(
        false
      );
    },
    SLOW
  ...
  it(
    "does not accept a hash whose stored digest was swapped",
    async () => {
      ...
      await expect(
        verifyPassword("my password is this", frankenstein)
      ).resolves.toBe(false);
      await expect(
        verifyPassword("their password is that", frankenstein)
      ).resolves.toBe(false);
    },
    SLOW
  );
...
describe("needsRehash", () => {
  it(
    "is false for a hash made with the current parameters",
    async () => {
  ...
  it("is true for a weaker cost", () => {
    expect(
      needsRehash("scrypt$16384$8$1$c2FsdHNhbHQ$aGFzaGhhc2hoYXNoaGFzaA")
    ).toBe(true);
  });
